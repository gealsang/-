...
export default {
  ...
  methods: {
    ...
    // 加入购物车的回调函数
    async addShopcar() {
      // 1 发请求---前端带一些参数给服务器[需要村相互这些数据],存储成功了,没有给返回数据
      /**
       * 当前这里是派发一个action,也想服务器发请求,判断加入购物车是成功还是失败了,进行相应的操作
       * this.$store.dispatch('addOrUpdateShopCart',{skuId:this.$route.params.id,skuNum:this.skuNum});
       * 上面这行代码说白了:调用仓库中addOrUpdateShopCart
